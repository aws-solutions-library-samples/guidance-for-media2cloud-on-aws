...
describe('audio-analysis', async function () {
  /* fixtures */
  const context = {
  ...
  describe('#start-transcribe', function () {
    before(function () {
    });
    ...
    it('operation is start-transcribe', async function () {
      expect({
        uuid: 'uuid',
        operation: 'start-transcribe',
        status: 'COMPLETED',
      }.operation).to.be.equal('start-transcribe');
    });
  });
});
