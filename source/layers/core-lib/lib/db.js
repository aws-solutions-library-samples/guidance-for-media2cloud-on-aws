...
const { mxCommonUtils } = require("./mxCommonUtils");
...
class DB {
  ...
  async update(primaryValue, sortValue, attributes, merge = true) {
    ...
    const merged = X.sanitizeJson(
      X.cleansing(X.merge(original, attributes), {
  ...
  async scanIndex(data) {
    const missing = ["Name", "Key", "Value"].filter(
      (x) => data[x] === undefined
    );
    ...
    const keyConditions = {
      [data.Key]: {
        ...
        AttributeValueList: [data.Value],
      },
    };

    const queryFilter = (data.Conditions || []).reduce(
      (a0, c0) => ({
        ...a0,
        [c0.Key]: {
          ComparisonOperator: c0.ComparisonOperator || "EQ",
          AttributeValueList: [c0.Value],
        },
      }),
      undefined
    );
    ...
    const params = {
      ...
      ExclusiveStartKey:
        data.Token && JSON.parse(Buffer.from(data.Token, "base64").toString()),
    };
  ...
  async fetch(primaryValue, sortValue, projection) {
    ...
    if (projection) {
      ...
      names = names.filter(
        (x) => x !== this.partitionKey && x !== this.sortKey
      );

      params.ExpressionAttributeNames = names.reduce(
        (acc, cur, idx) =>
          Object.assign(acc, {
            [`#p${idx}`]: cur,
          }),
      ...
      params.ProjectionExpression = Object.keys(
        params.ExpressionAttributeNames
      ).join(", ");
    }
  ...
  async dropColumns(primaryValue, sortValue, attributes) {
    ...
    items = items.filter((x) => x !== this.partitionKey && x !== this.sortKey);
