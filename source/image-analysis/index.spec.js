...
describe('image-analysis', async function () {
  /* fixtures */
  const context = {
  ...
  describe('#start-image-analysis', function () {
    before(function () {
    });
    ...
    it('operation is start-image-analysis', async function () {
      expect({
        uuid: 'uuid',
        operation: 'start-image-analysis',
        status: 'COMPLETED',
      }.operation).to.be.equal('start-image-analysis');
    });
  });
});
