...
describe('document-analysis', async function () {
  /* fixtures */
  const context = {
  ...
  describe('#start-document-text', function () {
    before(function () {
    });
    ...
    it('should return uuid, operation, status', async function () {
      expect({
        uuid: 'uuid',
        operation: 'start-document-text',
        status: 'COMPLETED',
      }).to.be.an('object').that.has.keys('uuid', 'operation', 'status');
    });

    it('operation is start-document-text', async function () {
      expect({
        uuid: 'uuid',
        operation: 'start-document-text',
        status: 'COMPLETED',
      }.operation).to.be.equal('start-document-text');
    });
  });
});
