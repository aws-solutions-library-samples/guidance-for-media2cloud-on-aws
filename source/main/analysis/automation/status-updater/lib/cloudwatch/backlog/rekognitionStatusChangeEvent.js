...
class RekognitionStatusChangeEvent extends BacklogStatusChangeEvent {
  async process() {
    if (this.status !== STATUS_SUCCEEDED && this.status !== STATUS_FAILED) {
      console.error(`ERR: RekognitionStatusChangeEvent.process: ${this.status} status not handled`);
      return undefined;
    }
...
module.exports = RekognitionStatusChangeEvent;
