...
export default class SegmentTab extends BaseRekognitionTab {
  ...
  async createContent() {
    ...
    const container = $('<div/>').addClass('col-9 m-0 p-0 max-h36r')
      .append(col01)
      .append(col02);
    return container;
  }
  ...
  async createEDLButton() {
    ...
    const name = `${AnalysisTypes.Rekognition.Segment}.zip`;
    const bucket = this.media.getProxyBucket();
    const href = await this.media.getUrl(bucket, `${prefix}${name}`);
